...
import com.company.jmixpm.entity.DashboardProject;
import com.company.jmixpm.entity.Project;
import com.company.jmixpm.entity.Task;
import com.company.jmixpm.entity.TaskStatus;
...
public class DashboardService {
    ...
    private Integer getActualEfforts(UUID projectId) {
        return unconstrainedDataManager.loadValue("select sum(te.timeSpent) from pm_TimeEntry te " +
    ...
    private Float calculateCompleteness(List<Task> tasks) {
        ...
        float closedTasks = tasks.stream()
                .filter(task -> TaskStatus.INACTIVE.equals(task.getStatus()))
                .toList()
                .size();
